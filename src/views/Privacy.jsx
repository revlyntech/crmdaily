...
export default function Privacy() {
  return (
    <>
      <style>{`
        .privacy-hero { background: #0F172A; padding: 80px 32px; }
        .privacy-hero h1 { font-family: 'DM Serif Display',serif; font-size: 64px; color: #FAFBFC; letter-spacing: -0.02em; }
        .privacy-body { background: #FAFBFC; padding: 64px 32px 96px; }
          ...
          <div style={{ maxWidth:760, margin:"0 auto" }}>
            {sections.map(([title,text],i) => (
              <motion.div key={title} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:i*0.07}}
                ...
                <span style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:"#E85D3A", letterSpacing:"0.15em", display:"block", marginBottom:10 }}>
                  {String(i+1).padStart(2,"0")} // {title.toUpperCase().replace(/ /g,"_")}
                </span>
                <h2 style={{ fontFamily:"'DM Serif Display',serif", fontSize:24, color:"#0F172A", marginBottom:12 }}>{title}</h2>
                <p style={{ fontFamily:"'Space Mono',monospace", fontSize:12, color:"#64748B", lineHeight:1.85 }}>{text}</p>
              </motion.div>
            ))}
          </div>
        </div>
