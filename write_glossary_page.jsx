...
export default function GlossaryPage() {
  ...
  const categoryCounts = useMemo(() => {
    ...
    for (const c of GLOSSARY_CATEGORIES) {
      m[c] = GLOSSARY_TERMS.filter(t => t.category === c).length;
    }
    return m;
  }, []);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFBFC', color: '#0F172A', fontFamily: "'Inter', sans-serif" }}>
      <style>{\`
        ...
        .term-card:hover { transform: translateY(-2px); box-shadow: 4px 4px 0 0 #E85D3A; }
        .term-card { transition: transform 0.2s, box-shadow 0.2s; }
        .letter-jump:hover { background: #E85D3A; color: #fff; }
      \`}</style>
      ...
      <div style={{ background: '#0F172A', color: '#FAFBFC', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '12px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: 'rgba(250,251,252,0.5)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>
            CRM 101 &middot; Glossary &middot; Vol. 01
          </span>
        </div>
      </div>

      <div style={{ background: '#E85D3A', overflow: 'hidden', borderBottom: '1px solid rgba(0,0,0,0.1)', padding: '8px 0' }}>
        <div className="marquee">
          {[...GLOSSARY_TERMS.filter(t => t.seenIn && t.seenIn.length > 0).slice(0, 14), ...GLOSSARY_TERMS.filter(t => t.seenIn && t.seenIn.length > 0).slice(0, 14)].map((t, i) => (
            <Link key={t.slug + i} href={"/glossary/" + t.slug}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, margin: '0 24px', color: '#fff', textDecoration: 'none', fontFamily: "'Space Mono', monospace", fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(255,255,255,0.7)', display: 'inline-block' }} />
              {t.term} <span style={{ opacity: 0.6 }}>/ {t.category}</span>
            </Link>
          ))}
        </div>
      </div>

      <div style={{ borderBottom: '2px solid #0F172A', background: '#FAFBFC' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '80px 32px', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 48 }}>
          <div>
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 96, fontWeight: 700, lineHeight: 0.95, color: '#0F172A', margin: '16px 0' }}>
              The<br />Glossary<span style={{ color: '#E85D3A' }}>.</span>
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: 'rgba(15,23,42,0.7)', maxWidth: 520, marginBottom: 40 }}>
          ...
          <div style={{ border: '2px solid #0F172A', background: '#FAFBFC', padding: 32 }}>
            ...
            {GLOSSARY_CATEGORIES.map(c => (
              <button key={c} onClick={() => setCat(cat === c ? 'All' : c)}
                style={{ display: 'flex', width: '100%', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid ' + (cat===c ? '#E85D3A' : 'rgba(15,23,42,0.1)'), background: 'none', cursor: 'pointer', textAlign: 'left' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ position: 'relative', flex: 1 }}>
              ...
              <input type="search" value={q} onChange={e => setQ(e.target.value)}
                placeholder={"Search " + GLOSSARY_TERMS.length + " terms, aliases, definitions..."}
                style={{ width: '100%', border: '2px solid #0F172A', background: '#FAFBFC', padding: '14px 120px 14px 44px', fontFamily: "'DM Serif Display',serif", fontSize: 18, outline: 'none', boxSizing: 'border-box' }} />
              <span style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', fontFamily: "'Space Mono',monospace", fontSize: 11, color: 'rgba(15,23,42,0.5)', fontWeight: 700 }}>
                {String(filtered.length).padStart(3,'0')} / {String(GLOSSARY_TERMS.length).padStart(3,'0')}
              </span>
            </div>
            {(q || cat !== 'All') && (
              <button onClick={() => { setQ(''); setCat('All'); }}
                style={{ border: '2px solid #0F172A', background: '#0F172A', color: '#FAFBFC', padding: '12px 16px', fontFamily: "'Space Mono',monospace", fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>
                Clear X
              </button>
            )}
          </div>
          <div style={{ marginTop: 16, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {['All', ...GLOSSARY_CATEGORIES].map(c => (
                <button key={c} onClick={() => setCat(c)}
                  style={{ border: '2px solid', borderColor: cat===c ? '#E85D3A' : '#CBD5E1', background: cat===c ? '#E85D3A' : '#FAFBFC', color: cat===c ? '#fff' : 'rgba(15,23,42,0.7)', padding: '6px 12px', fontFamily: "'Space Mono',monospace", fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', display: 'inline-flex', gap: 6 }}>
            ...
            <nav style={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              {LETTERS.map(L => {
                ...
                return (
                  <a key={L} href={has ? "#letter-" + L : undefined} className={has ? 'letter-jump' : ''}
                    style={{ width: 28, padding: '4px 0', textAlign: 'center', fontFamily: "'DM Serif Display',serif", fontSize: 14, fontWeight: 700, color: has ? '#0F172A' : 'rgba(15,23,42,0.2)', cursor: has ? 'pointer' : 'not-allowed', textDecoration: 'none', display: 'inline-block' }}>
                    {L}
                  </a>
                );
              })}
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '64px 32px' }}>
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ fontSize: 18, color: 'rgba(15,23,42,0.6)', marginTop: 16 }}>No terms match &ldquo;{q}&rdquo;</p>
            <button onClick={() => { setQ(''); setCat('All'); }}
              style={{ marginTop: 24, border: '2px solid #0F172A', background: 'none', padding: '10px 24px', fontFamily: "'Space Mono',monospace", fontSize: 11, fontWeight: 700, textTransform: 'uppercase', cursor: 'pointer' }}>
              Reset filters
            </button>
          </div>
        )}
          ...
          <div key={letter} id={"letter-" + letter} style={{ marginBottom: 80, scrollMarginTop: 160 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', alignItems: 'end', gap: 24, borderBottom: '2px solid #0F172A', paddingBottom: 16, marginBottom: 32 }}>
              ...
              <div style={{ textAlign: 'right', paddingBottom: 12 }}>
                <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, fontWeight: 700, color: 'rgba(15,23,42,0.5)', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
                  {String(terms.length).padStart(2,'0')} {terms.length === 1 ? 'term' : 'terms'}
                </span>
                <div style={{ marginTop: 4, height: 4, width: 96, background: '#E85D3A', marginLeft: 'auto' }} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
              {terms.map(t => {
                ...
                return (
                  <Link key={t.slug} href={"/glossary/" + t.slug} className="term-card"
                    style={{ border: '2px solid #0F172A', background: '#FAFBFC', padding: 20, display: 'block', textDecoration: 'none', color: '#0F172A' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 12 }}>
                      <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, fontWeight: 700, color: 'rgba(15,23,42,0.4)' }}>No {String(runningIndex).padStart(3,'0')}</span>
                      <span style={{ border: '1px solid #CBD5E1', padding: '2px 8px', fontFamily: "'Space Mono',monospace", fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(15,23,42,0.6)' }}>{cm.code} {t.category}</span>
                    </div>
                    <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 22, fontWeight: 700, margin: '12px 0 4px' }}>{t.term}</div>
                    {t.aliases && t.aliases.length > 0 && (
                      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(15,23,42,0.4)', marginBottom: 8 }}>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #E8ECF1', display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(15,23,42,0.4)' }}>Read definition</span>
                      <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: 18, color: 'rgba(15,23,42,0.4)' }}>&rarr;</span>
                    </div>
                  </Link>
                );
              })}
      ...
      <div style={{ borderTop: '2px solid #0F172A', background: '#0F172A', color: '#FAFBFC', padding: '80px 32px' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32, alignItems: 'end' }}>
          <div>
            ...
            <p style={{ fontSize: 18, color: 'rgba(250,251,252,0.7)', maxWidth: 520, lineHeight: 1.7 }}>
              If it's in a deal review, a board deck or an AE Slack thread and it's not in here yet, the desk wants to know.
            </p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <a href="mailto:hello@crmdaily.co?subject=Glossary%20suggestion"
              style={{ display: 'flex', justifyContent: 'space-between', border: '2px solid #FAFBFC', background: '#E85D3A', padding: '16px 24px', fontFamily: "'Space Mono',monospace", fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#fff', textDecoration: 'none' }}>
              Email the desk <span>&rarr;</span>
            </a>
            <Link href="/"
              style={{ display: 'flex', justifyContent: 'space-between', border: '2px solid rgba(250,251,252,0.3)', padding: '16px 24px', fontFamily: "'Space Mono',monospace", fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(250,251,252,0.7)', textDecoration: 'none' }}>
...
fs.writeFileSync(path.join('src', 'app', 'glossary', 'page.jsx'), content, 'utf8');
console.log('Done! page.jsx written successfully');
